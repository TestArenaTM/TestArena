...
*/
class Report_PhaseController extends Custom_Controller_Action_Application_Project_Abstract
{
  ...
  public function indexAction()
  {
    ...
    $phaseMapper = new Report_Model_PhaseMapper();
    list($list, $paginator) = $phaseMapper->getAll($this->getRequest());
  ...
  {
    $phaseMapper = new Report_Model_PhaseMapper();
    $rows = $phaseMapper->getAllForExport($this->getRequest());
    ...
    foreach ($rows as $row)
    {
      ...
      if ($row['name'] === null)
      {
        $row['name'] = $t->translate('defaultPhaseName', null, 'general');
      }
