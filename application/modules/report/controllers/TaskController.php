...
*/
class Report_TaskController extends Custom_Controller_Action_Application_Project_Abstract
{
  ...
  {
    $taskMapper = new Report_Model_TaskMapper();
    $this->_setTranslateTitle();
    $this->view->tasks = $taskMapper->getByType($this->getRequest());
  }
  
  public function exportAllByTypeAction()
  {
    $taskMapper = new Report_Model_TaskMapper();
    $rows = $taskMapper->getAllForExport($this->getRequest());
    ...
    $types = new Application_Model_TaskType();
    $types = $types->getNames();
    
    foreach ($rows as $row)
    {
      $row['name'] = $t->translate('TASK_'.$types[$row['type']], null, 'type');
