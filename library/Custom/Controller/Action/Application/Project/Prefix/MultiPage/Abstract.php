...
*/
class Custom_Controller_Action_Application_Project_Prefix_MultiPage_Abstract extends Custom_Controller_Action_Application_Project_Prefix_Abstract
{
  ...
  public function indexAction()
  {
    ...
    $form = $this->getForm()->prepareSubForm($form, $this->view->linkRoute()->getUrl('release_clone_multiform_process', false, array('id' => $this->getRequest()->getParam('id')), false, true), false, $this->checkAttachSubmitButton($form->getName()), $this->hasRemainingFormsLeft());
    $form = $this->populateForm($form, $formSessionData);
    
    if ($form instanceof Project_Form_ReleaseCloneStepOne)
    {
      $environmentMapper = new Project_Model_EnvironmentMapper();
      $versionMapper = new Project_Model_VersionMapper();

      $this->view->prePopulatedEnvironments = $form->stepOne->prePopulateEnvironments($environmentMapper->getForPopulateByIds($form->stepOne->getEnvironments()));
      $this->view->prePopulatedVersions = $form->stepOne->prePopulateVersions($versionMapper->getForPopulateByIds($form->stepOne->getVersions()));
    }
