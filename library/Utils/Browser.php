...
class Utils_Browser
{
  ...
  private $_agentLanguage;
  private $_agentLanguageTag;
  private $_osType;
  private $_osName;
  private $_osVersionName;
  private $_osVersionNumber;
  private $_osProducer;
  private $_osProducerURL;
  private $_linuxDistibution;
  ...
  {
    $result = $this->_runUrl('http://www.useragentstring.com/?uas='.urlencode(self::getServerValue('HTTP_USER_AGENT')).'&getText=all');
    $items = explode(';', $result);
    $vars = get_class_vars('Utils_Browser');
    ...
    foreach ($items as $item)
    {
      ...
      {
        $buf = explode('=', $item, 2);
        $varName = '_'.Utils_Text::createCamelHumps($buf[0]);
  ...
  public static function getServerValue($name)
  {
  ...
  public function getLinuxDistibution()
  {
    return $this->_linuxDistibution;
  }
    
  private function _runUrl($url)
  {
    $ch = curl_init($url);
